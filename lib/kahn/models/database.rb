  ...
  module Models
    class Database
      ...
      def create_collection(coll_name)
        raise Kahn::CollectionAlreadyExists.new(self, coll_name) if database.collection_names.include? coll_name
        database.create_collection coll_name rescue raise Kahn::MalformedDocument.new('Invalid collection name')
      ...
      def [](coll_name)
        raise Kahn::CollectionNotFound.new(self, coll_name) unless database.collection_names.include? coll_name
        Collection.new(database[coll_name])
      end
