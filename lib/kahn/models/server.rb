  ...
  module Models
    class Server
      ...
      def create_database(db_name)
        raise Kahn::DatabaseAlreadyExists.new(self, db_name) if db_exists? db_name
        begin
          client[db_name]['__kahn_tmp_collection__'].drop
        rescue Mongo::InvalidNSName
          raise Kahn::MalformedDocument.new('Invalid database name')
        end
      ...
      def [](db_name)
        raise Kahn::DatabaseNotFound.new(self, db_name) unless db_exists? db_name
        Database.new(client, db_name)
      end
